...
local BlipCheck = false
local NameCheck = false

RegisterNetEvent('qb-admin:client:blips', function()
    ShowBlips = not ShowBlips
    BlipCheck = ShowBlips or BlipCheck
    ShowNamesBlips()
    local message = ShowBlips and Lang:t('success.blips_activated') or Lang:t('error.blips_deactivated')
    lib.notify({ description = message, type = ShowBlips and 'success' or 'error' })
end)

RegisterNetEvent('qb-admin:client:names', function()
    ShowNames = not ShowNames
    NameCheck = ShowNames or NameCheck
    ShowNamesBlips()
    local message = ShowNames and Lang:t('success.names_activated') or Lang:t('error.names_deactivated')
    lib.notify({ description = message, type = ShowNames and 'success' or 'error' })
end)

local threadRunning = false
function ShowNamesBlips()
    if threadRunning then return end
    CreateThread(function()
        threadRunning = true
        while BlipCheck or NameCheck do
            local players = lib.callback('qb-admin:server:getplayers')
